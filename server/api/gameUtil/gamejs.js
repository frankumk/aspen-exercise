...
class Game {
  ...
  playHand() {
    const play = [this.playerOneDeck.shift()] //player one discard
    play.push(this.playerTwoDeck.shift()) //player two discard
    const handResult = this.determineRoundWinner(
      this.playerOneDeck,
      this.playerTwoDeck,
      play,
    )
  ...
  async checkGameState() {
    if (this.p1CardCount() <= 0) {
      ...
      console.log('game play continues')
      return
    }
  }
}
